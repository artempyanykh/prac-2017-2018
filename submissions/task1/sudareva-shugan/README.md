...
По ходу программы в функции  `nash_equilibrium(a)` вызывается функция  `graf(a, b)`. Внутри функции graf вызывается команда plt.figure()(добавление на рисунок прямоугольной (по умолчанию) области рисования), команда plt.scatter()(она ставит маркер значений вектора оптимальных стратегий) вызывается в цикле, команда plt.grid() строит линии вспомогательной сетки, plt.show() показывает рисунок. 
...
2 часть задания - визуализация спектра оптимальных стратегий: написание функции `graf(a, b)` и подбор матриц для демонтстрации трех различных игр, согласно условию задачи.
