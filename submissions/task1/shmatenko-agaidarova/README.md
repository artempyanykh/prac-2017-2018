...
Реализация решения
----------------
На вход функции nash_equilibrium() подаётся матрица mxn, где m - число стратегий первого игрока, n - второго.
...
`figure(), scatter(), _label(), grid(), show()`
...
- `_label` - позволяет помечать координатные оси
