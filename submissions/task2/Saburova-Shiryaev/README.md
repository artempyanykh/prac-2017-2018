 ...
 *Временной ряд* - совокупность значений какого-либо показателя за определенное время.
 ...
 *Сезональностью* временного ряда называются периодические колебания, наблюдаемые во временных рядах.
 ...
 *AR (autoregressive model)* - авторегрессионная модель временных рядов, в которой значения временого ряда в данных моемент зависит от предыдущих значений этого же ряда.
...
3. Разложение временного ряда на тренд, сезональность остаток в соответствии с аддитивной и мультипликативной моделями
    * `decomposition(data, modelka, title)` реализует разложение на соответствующие модели
		+ data - временной ряд
		+ modelka - параметр для функции decomp, которая осуществляет разложение временного ряда
...
6. ARIMA-модель
	...
	* `model.fit()` - Прогоняем ARIMA-модель через фильтр Калмана
		+  Фильтр Калмана - мощнейщий инструмент фильтрации данных. При фильтрации используется информация о физике самого явления.
		...
		+ Метод класса ARIMA
	* `model.predict(start='1989-01-01', end='1993-12-01', typ='levels', dynamic=True)` - строим предсказание на тестовой выборке
...
7) SARIMAX
	* `SARIMAX(data, order=(p, d, q), seasonal_order=(p,d,q,s))` - строит модель SARIMAX на основе временного ряда ts
		+ data - временной ряд
		...
		+  Сезональность (применительно к данной модели) - число периодов в сезоне (12 - для ежемесячных данных)
