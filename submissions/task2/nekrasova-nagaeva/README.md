...
Эргодичность - свойство процесса, позволяющее для оценки мат.ожиданий использовать усреднения по времени.  
Для того, чтобы стационарный процесс был эргодичным, достаточно выполнения k = \[1,T) : (T^(-1)\*∑ γk) -> 0 при T -> ∞

Подход Бокса-Дженкинса к построению модели типа ARIMA:  
1a. Остационарить ряд, установив порядок интеграции d;  
1b. Исходя из поведения автокорреляционной и частной автокорреляционной функций установить параметры p и q;  
2. Оценивание коэффициентов  α1, α2, ... , αp, β1, β2, ... , βq;
3. По остаткам осуществляется тестирование или диагностика построенной модели;
4. Прогнозирование.
...
RSS = ∑ (x(t) - x'(t))^2, t ∈ \[1, n], где x(t) - фактические значения объясняемой переменной, x'(t) - расчетные значения.
